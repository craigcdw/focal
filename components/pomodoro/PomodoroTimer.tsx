...
export function PomodoroTimer() {
  ...
  const phaseColor = phase === "work" ? "#3b82f6" : phase === "short_break" ? "#10b981" : "#8b5cf6";
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-semibold text-gray-900 dark:text-white tracking-tight">Focus</h1>
          <p className="text-gray-500 dark:text-zinc-400 mt-1">{todaySessions.length} sessions today</p>
        </div>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <div className="flex gap-3 mt-4">
              {(["work", "short_break", "work", "short_break"] as const).slice(0, settings.sessions_before_long_break).map((_, i) => (
                <div key={i} className={`w-2 h-2 rounded-full transition-colors ${i < (sessionsCompleted % settings.sessions_before_long_break) ? "bg-blue-500" : "bg-gray-200 dark:bg-zinc-700"}`} />
              ))}
            </div>
          </div>
        </div>
