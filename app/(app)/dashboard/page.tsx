...
export default async function DashboardPage() {
  ...
  const PRIORITY_DOT: Record<string, string> = {
    urgent: "bg-[#0D1B2A] dark:bg-white",
    high:   "bg-[#5C6370]",
    medium: "bg-[#aeaeb2]",
    low:    "bg-[#d1d1d6]",
  };
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      <div>
        ...
        <h1 className="text-3xl font-bold text-[#0D1B2A] dark:text-white tracking-tight">
          {greeting()}, {name}.
        </h1>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 border border-gray-100 dark:border-zinc-800">
          <p className="text-xs font-semibold text-[#5C6370] dark:text-zinc-500 uppercase tracking-widest mb-4">Active tasks</p>
          {tasks && tasks.length > 0 ? (
            <div className="space-y-0">
              {tasks.slice(0, 6).map((t) => (
                <div key={t.id} className="flex items-center gap-3 py-2.5 border-b border-[#F5F4EE] dark:border-zinc-800 last:border-0">
                  <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${PRIORITY_DOT[t.priority] ?? "bg-[#d1d1d6]"}`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-[#0D1B2A] dark:text-white truncate">{t.title}</p>
                  </div>
                  {t.due_date && (
                    <p className="text-xs text-[#5C6370] dark:text-zinc-500 flex-shrink-0">
            ...
            <p className="text-sm text-[#5C6370] dark:text-zinc-500">No active tasks — nice work!</p>
          )}
        </div>

        {/* Upcoming events */}
        <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 border border-gray-100 dark:border-zinc-800">
          <p className="text-xs font-semibold text-[#5C6370] dark:text-zinc-500 uppercase tracking-widest mb-4">Upcoming events</p>
          {events && events.length > 0 ? (
            <div className="space-y-0">
              {events.map((e) => (
                <div key={e.id} className="flex items-center gap-3 py-2.5 border-b border-[#F5F4EE] dark:border-zinc-800 last:border-0">
                  <div className="w-1 self-stretch rounded-full flex-shrink-0" style={{ backgroundColor: e.color }} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#0D1B2A] dark:text-white truncate">{e.title}</p>
            ...
            <p className="text-sm text-[#5C6370] dark:text-zinc-500">No upcoming events</p>
          )}
        </div>

        {/* Recent notes */}
        <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 border border-gray-100 dark:border-zinc-800">
          <p className="text-xs font-semibold text-[#5C6370] dark:text-zinc-500 uppercase tracking-widest mb-4">Recent notes</p>
          {notes && notes.length > 0 ? (
            <div className="space-y-0">
              {notes.map((n) => (
                <div key={n.id} className="flex items-center justify-between py-2.5 border-b border-[#F5F4EE] dark:border-zinc-800 last:border-0">
                  <p className="text-sm text-[#0D1B2A] dark:text-white truncate">{n.title}</p>
                  <p className="text-xs text-[#5C6370] dark:text-zinc-500 ml-4 flex-shrink-0">
            ...
            <p className="text-sm text-[#5C6370] dark:text-zinc-500">No notes yet</p>
          )}
        </div>

        {/* Today's focus */}
        <div className="bg-[#0D1B2A] dark:bg-zinc-800 rounded-2xl p-6 border border-transparent">
          <p className="text-xs font-semibold text-[#D4A017] uppercase tracking-widest mb-4">Today&apos;s focus</p>
