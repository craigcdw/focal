...
const CATEGORY_COLORS: Record<Category, string> = {
  shoulder: "bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300",
  upper:    "bg-purple-100 dark:bg-purple-950 text-purple-700 dark:text-purple-300",
  lower:    "bg-green-100 dark:bg-green-950 text-green-700 dark:text-green-300",
  core:     "bg-orange-100 dark:bg-orange-950 text-orange-700 dark:text-orange-300",
  cardio:   "bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-300",
  stretch:  "bg-teal-100 dark:bg-teal-950 text-teal-700 dark:text-teal-300",
};
...
function EquipmentBadge({ eq }: { eq: Equipment }) {
  const colors: Record<Equipment, string> = {
    mat: "bg-teal-50 dark:bg-teal-950 text-teal-700 dark:text-teal-300",
    dumbbells: "bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-300",
    chair: "bg-yellow-50 dark:bg-yellow-950 text-yellow-700 dark:text-yellow-300",
    wall: "bg-stone-100 dark:bg-stone-900 text-stone-600 dark:text-stone-300",
    bodyweight: "bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-300",
  };
  return (
...
function ExerciseCard({ ex, index }: { ex: Exercise; index: number }) {
  ...
  return (
    <div className={`rounded-2xl border transition-all ${open ? "border-blue-200 dark:border-blue-800 bg-blue-50/30 dark:bg-blue-950/20" : "border-gray-100 dark:border-zinc-800 bg-white dark:bg-zinc-900"}`}>
      <button
        onClick={() => setOpen(o => !o)}
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-gray-100 dark:border-zinc-800 pt-3">
          ...
          {ex.shoulderNote && (
            <div className={`flex items-start gap-2 p-3 rounded-xl text-xs leading-relaxed ${ex.avoid ? "bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300" : "bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300"}`}>
              {ex.avoid
...
function WorkoutDetail({ workout, onBack }: { workout: Workout; onBack: () => void }) {
  return (
    <div className="space-y-6">
      ...
      <div className={`bg-gradient-to-br ${workout.color} rounded-2xl p-6 text-white`}>
        <p className="text-sm font-medium opacity-80 mb-1">{workout.subtitle}</p>
        <h2 className="text-2xl font-bold mb-1">{workout.title}</h2>
        <p className="text-sm opacity-90 mb-4">{workout.tagline}</p>
        <div className="flex items-center gap-4 text-sm">
          <span className="flex items-center gap-1.5 bg-white/20 px-3 py-1 rounded-full">
            <Clock size={13} /> {workout.duration}
          </span>
          <span className="bg-white/20 px-3 py-1 rounded-full">{workout.difficulty}</span>
          <span className="bg-white/20 px-3 py-1 rounded-full">{workout.exercises.length} exercises</span>
        </div>
      </div>
...
export function WorkoutsView() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-2xl p-4 flex items-start gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {WORKOUTS.map(w => (
          ...
          >
            <div className={`bg-gradient-to-r ${w.color} p-5 text-white`}>
              <p className="text-xs font-medium opacity-80 mb-0.5">{w.subtitle}</p>
